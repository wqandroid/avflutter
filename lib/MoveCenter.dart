...
//  }
  ...
  Future getMoveDetailInfo(String link)async{
    ...
    try{
      ...
      moveDetailInfo.coverUrl=document.querySelector("[class=bigImage]").attributes["href"];


      List<Element> boxs = document.querySelectorAll("[class*=sample-box]");
      boxs.forEach((box){
        String thumbnailUrl=box.getElementsByTagName("img")[0].attributes["src"];
      ...
      List<Element> attresses = document.querySelectorAll("[class*=avatar-box]");

      print("演员信息:${attresses.length}");

      attresses.forEach((box){
        moveDetailInfo.actresses.add(new Actress(box.text,
...
//        });
        ...
        int size= headerNames.length<headerAttr.length?headerNames.length:headerAttr.length;
        for(int i=0;i<size;i++){
          moveDetailInfo.headers.add(Header(headerNames[i], headerAttr[i][0].trim(),  headerAttr[i][1].trim()));
        }
